...
function TransferGudang({ user, onLogout }) {
  ...
  const gudangSama =
    header.from_lokasi_id && header.from_lokasi_id === header.to_lokasi_id
      ...
      <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-2xl p-6 mb-8">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            ...
            <select
              className={`${input} ${gudangSama ? 'border-accent' : ''}`} required
              value={header.to_lokasi_id}
            ...
            {gudangSama && (
              <p className="mt-1 text-[11px] text-accent">Pilih gudang tujuan yang berbeda.</p>
            )}
          </div>
          <div>
                ...
                <div key={i} className="grid grid-cols-12 gap-3 items-end">
                  <div className="col-span-12 sm:col-span-7">
                    <select
                      ...
                      disabled={!header.from_lokasi_id}
                    >
        ...
        <button
          type="submit"
          disabled={saving || gudangSama || adaYangKurang}
              ...
              riwayat.map((t) => (
                <Fragment key={t.id}>
                  <tr className="border-b border-border hover:bg-surface-soft transition-colors">
                    ...
                    <td className="px-5 py-3 font-mono">{t.total_qty}</td>
